...
<div class="min-h-screen">
    <div class="max-w-5xl mx-auto px-4">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-slate-900">Registration Fields</h1>
            <a href="{{ route('registration-fields.create') }}"
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6">
            <h2 class="text-lg font-semibold text-slate-900 mb-4">All Registration Fields</h2>
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left font-semibold text-slate-700">Segment</th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($fields as $field)
                            <tr>
                                ...
                                <td class="px-3 py-2">
                                    @if($field->is_system)
                                        <span class="inline-flex items-center rounded-full bg-violet-100 px-2 py-1 text-xs font-medium text-violet-700">
                                            System
                                        </span>
                                    @else
                                ...
                                <td class="px-3 py-2 text-right">
                                    <a href="{{ route('registration-fields.edit', $field) }}"
                                    ...
                                    <form action="{{ route('registration-fields.destroy', $field) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                onclick="return confirm('Delete this registration field?')"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-3 py-4 text-center text-slate-500">No registration fields found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
